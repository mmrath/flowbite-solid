...
import {Component, ComponentProps, createMemo, JSX, mergeProps, splitProps, useContext} from "solid-js";
...
export const AccordionTitle: Component<AccordionTitleProps> = (p:AccordionTitleProps): JSX.Element => {
  const [local, props] = splitProps(mergeProps({as:'h2',theme:{}}, p), ["as", "children", "class", "theme", "onClick"])
  ...
  const theme = createMemo(
      ()=> mergeDeep(defaultTheme.theme.accordion.title, local.theme)
  );
  ...
  return (
    <button
      ...
      {...props}
    >
      <Dynamic component={local.as} class={theme().heading} data-testid="flowbite-accordion-heading">
        {local.children}
      </Dynamic>
      {accordionContext.arrowIcon && (
        <Dynamic component={accordionContext.arrowIcon}
          aria-hidden
          class={classNames(theme().arrow.base, theme().arrow.open[itemContext.isOpen ? 'on' : 'off'])}
