...
import {Component, ComponentProps, createMemo, createSignal, createUniqueId, mergeProps, splitProps} from "solid-js";
...
export const ToggleSwitch: Component<ToggleSwitchProps> = (p) => {
    ...
    const [local, props] = splitProps(mergeProps(defaultProps, p),
        ["checked", "disabled", "color", "label", "name", "onChange", "class", "theme"]);
    ...
    return (
        <>
            {local.name && local.checked &&
                <input checked={checked()} hidden name={local.name} readOnly type="checkbox" class="sr-only"/>}
            <button
