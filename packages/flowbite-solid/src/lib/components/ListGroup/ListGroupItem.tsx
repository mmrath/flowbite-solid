...
export const ListGroupItem: Component<ListGroupItemProps> = (p): JSX.Element => {
    const defaultProps = {};
    const [local, props] = splitProps(mergeProps(defaultProps, p), ["active", "children", "href", "icon", "onClick", "class"]);
    ...
    return (
        <li>
            <Dynamic component={isLink() ? 'a' : 'button'}
                     class={classNames(theme().active[local.active ? 'on' : 'off'], theme().base, theme().href[isLink() ? 'on' : 'off'])}
                     href={local.href}
