...
import {Component, ComponentProps, createEffect, createMemo, JSX, mergeProps, splitProps, useContext} from "solid-js";
...
export const AccordionTitle: Component<AccordionTitleProps> = (p: AccordionTitleProps): JSX.Element => {
    const [local, props] = splitProps(mergeProps({
        as: 'h2',
        theme: {}
    }, p), ["as", "children", "class", "theme", "onClick"])
    ...
    const theme = createMemo(
        () => mergeDeep(useTheme().theme.accordion.title, local.theme)
    );
    createEffect(() => {
