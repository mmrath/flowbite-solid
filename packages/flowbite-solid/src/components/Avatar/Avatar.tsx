...
const AvatarComponent: Component<AvatarProps> = p => {
  ...
  const imgProps = createMemo(() => {
    return {
      alt: local.alt,
      class: classNames(imgClassName(), theme().img.on),
    ...
    >
      <div class="relative">
        <Switch
          fallback={
            <div
              class={classNames(imgClassName(), theme().img.off)}
              data-testid="flowbite-avatar-img"
            >
              <svg
        ...
        >
          ...
          <Match when={local.placeholderInitials}>
            ...
            >
              <span
                class={classNames(theme().initials.text)}
